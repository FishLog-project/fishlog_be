...
└─ global
   ...
   ├─ congestion                              # 관광지 집중률(당일 혼잡도) 외부연동 — CongestionClient(+Impl)·CongestionService(+Impl)·dto/CongestionRate. 시군구 1콜 → 당일 만료 Redis 캐시, 실패는 빈 맵(목록 200 유지). 에러코드는 TourErrorCode 재사용 → docs/external.md §2-1
...
| `congestion` | 관광지 집중률(당일 혼잡도) 외부연동 — `CongestionClient`(+`Impl`)·`CongestionService`(+`Impl`)·`dto/CongestionRate`. 시군구 단위 1콜 → 당일 만료 Redis 캐시, 실패는 빈 맵으로 삼켜 관광 목록 200 유지. 에러 코드는 `TourErrorCode` 재사용 → docs/external.md §2-1 | ✅ |
