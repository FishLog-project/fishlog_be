...
> **⛔ 커밋·푸시는 사용자 명시 지시가 있을 때만:** 작업(코드/문서 수정)을 완료해도 **사용자가 명시적으로 요청하기 전에는 `git commit`·`git push`를 실행하지 않습니다.** 변경은 워킹 트리에 남겨 두고, 사용자에게 커밋 여부를 확인합니다. `git add`를 포함한 스테이징도 요청 전 선제 실행하지 않습니다. (에이전트 자동 커밋 금지)
