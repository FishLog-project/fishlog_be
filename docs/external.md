...
- **관광지 하나씩(`tAtsNm`)이 아니라 시군구 통째로 1콜**을 받는다. 일 호출 한도가 1,000건이라 목록 30건에 30콜은 금방 소진되는 반면, 시군구 1콜(제주시 244곳 × 30일 = 7,320행)이면 그 결과가 목록 전체를 채운다.
- **캐시:** Redis `congestion:{signguCd}:{yyyyMMdd}`, TTL은 **KST 자정까지**. 날짜 단위 예측이라 날이 바뀌면 값이 통째로 무의미해지므로 12h 같은 고정 TTL을 쓰지 않는다(키에도 날짜를 넣어 이중 방어).
- **"데이터 없음"도 캐시한다**(전남 등). 매 요청 외부를 두드려도 영원히 빈 결과다. 단 **호출 실패는 캐시하지 않는다** — 일시 장애를 하루 붙잡으면 안 된다.
...
2. **이름 매칭 적중률 약 57%** (전남 제외, 낚시 스팟 6곳 주변 관광지 69건). 지역별: 통영 13/15, 태안 5/6, 제주 한림 5/7, 속초 8/15, 부안 5/11, 해운대 3/15.
...
- [ ] 집중률 일 호출 한도(1,000건) 실제 소진 추이 관측 — 시군구 캐시로 충분한지
