...
> ⚠️ **집중률도 동일하게 실시간·무캐시다**(§2-1). 관광지 목록 요청 1건 = TourAPI 1콜 + 시군구 수만큼 집중률 콜(보통 1~2). 쿼터 소진 시 혼잡도만 `null`로 떨어지고 목록은 200을 유지한다.
...
### 2-1. 관광지 당일 혼잡도 — 집중률 예측 API ✅

- **용도:** 주변 관광 장소 조회(`GET /api/tours/nearby`) 응답의 각 관광지에 **그날 예상 혼잡도**를 덧붙인다. 지도 마커를 눌렀을 때 보여주는 값.
- **제공처:** 한국관광공사_관광지 집중률 방문자 추이 예측 정보 (공공데이터포털 **15128555**)
...
집중률 API의 필수 요청값 3개가 모두 `locationBasedList2` 응답 안에 있다. **프론트 요청 파라미터 변경도, 지역코드 매핑 테이블도 필요 없다.**

| 집중률 요청 파라미터 | 값 | TourAPI 응답 출처 |
|---|---|---|
| `serviceKey` | 발급키 | (동일 키 재사용) |
| `MobileOS` / `MobileApp` | `ETC` / `fishlog` | 필수 |
| `_type` | `json` | 응답 포맷 |
| `areaCd` | 법정동 시도코드 **2자리** (예 `52`) | **`lDongRegnCd`** |
| `signguCd` | 법정동 시군구코드 **5자리** (예 `52800`) | **`lDongRegnCd` + `lDongSignguCd`** |
| `tAtsNm` | 관광지명(부분일치 LIKE) | `title` |
| `numOfRows` / `pageNo` | 9999 / 1 | 시군구 통째로 1콜 |

> ⚠️ 같은 응답의 `areacode`/`sigungucode`는 **한국관광공사 자체 코드**(부안군 = `37`/`6`)라 집중률 API가 받지 않는다. 반드시 `lDong*` 쪽을 쓴다.

#### 응답 매핑

| 필드 | 의미 | 응답 |
|---|---|---|
| `baseYmd` | 예측 기준일 `yyyyMMdd` | 당일 행만 필터 → `congestion.baseDate` |
| `tAtsNm` | 관광지명 | 조인 키(응답에는 내보내지 않음) |
| `cnctrRate` | 집중률 지수 0~100 | `congestion.rate` (+ 등급 `congestion.level`) |
| `areaNm`/`signguNm` | 지역·시군구명 | 사용 안 함 |

- **`baseYmd`는 요청 파라미터가 아니다** — 넣으면 `INVALID_REQUEST_PARAMETER_ERROR(baseYmd)`. 항상 30일치가 오므로 **당일 필터는 서버에서** 한다.
- `cnctrRate`는 **퍼센트가 아니라 2018년 이후 피크를 100으로 정규화한 지수**다. 그대로 "혼잡도 39%"로 노출하지 않고 등급으로 바꿔 내보낸다.
- 등급 경계(`CongestionLevel`): **여유 <50 / 보통 50~80 / 혼잡 ≥80**. 실측(2026-09-19, 통영·속초·제주시·태안 485건)에서 중앙값 69.7, 70 초과 49%로 분포가 위로 쏠려 있어 33/66 3등분은 거의 전부를 "혼잡"으로 만든다.
- ⚠️ **정규화는 관광지별이다 — 장소끼리 비교하면 안 된다.** 부안군 41곳 중 20곳이 30일 내 95 이상에 도달하고(30일 최대값 중앙값 95.0), 부안향교 같은 소규모 관광지도 97.5까지 올라간다. 전체 기준 정규화라면 불가능한 값이다. 즉 `수성당 99.8`과 `상록해수욕장 84.7`은 "수성당에 사람이 더 많다"가 아니라 **각자 자기 역사적 피크 대비 수준**이다. 화면 문구를 "오늘 이 장소, 평소보다 붐빔" 쪽으로 고정해 마커 간 비교를 유도하지 않아야 한다.

#### 연동 방식 ✅ — 매 요청 실시간 호출 (저장·캐시 없음)

- 서버 런타임: **`global/congestion`** — `CongestionClient`(+`Impl`, 호출·파싱), `CongestionService`(+`Impl`, 당일 집중률 맵 캐시), `dto/CongestionRate`. 에러 코드는 **`TourErrorCode`를 재사용**한다(같은 기관·같은 키·같은 실패 양상).
- 이름 매칭 규칙은 **`domain/tour/policy/CongestionMatcher`** 에 모은다.
- ⛔ **Redis·DB에 저장하지 않고 매 요청 실시간 호출한다.** §2 TourAPI와 동일한 제품 제약이며, **관광데이터 활용 공모전 규칙**(관광공사 데이터를 적재해두고 그걸로 서빙 금지)이기도 하다. 쿼터를 아끼려고 캐시를 두면 "우리 저장소에서 서빙"이 되어 규칙에 어긋난다.
- 다만 **한 요청 안에서의 중복 호출은 제거한다** — 목록에 같은 시군구 장소가 여러 개여도 그 요청 동안 1콜이다(`TourServiceImpl`이 시군구별로 모아 호출). 이건 저장이 아니라 같은 요청 내 중복 제거라 실시간성을 해치지 않는다.
- **관광지 하나씩(`tAtsNm`)이 아니라 시군구 통째로 1콜**을 받는다. 목록 30건에 30콜을 쓰는 대신 시군구 1콜(제주시 244곳 × 30일 = 7,320행)이면 그 결과가 목록 전체를 채운다. 실시간성을 지키면서 호출 수를 줄이는 유일한 수단이다.
- **실패 격리:** 혼잡도는 부가 정보라 `CongestionService`가 예외를 삼키고 빈 맵을 반환한다. 외부 장애·쿼터 소진에도 **관광 목록 응답은 200을 유지**하고 `congestion: null`로만 떨어진다. (TourAPI 본 호출은 기존대로 502/503을 던진다.)
- `type=숙박`·`음식점`은 **외부 호출 자체를 생략**한다 — 집중률 데이터셋이 관광지만 다뤄 어차피 전부 매칭 실패다.

#### ⚠️ 확인된 커버리지 한계 (실측 2026-09-19)

1. **전남 전역 데이터 0건.** TourAPI는 이미 `전남광주통합특별시`(`lDongRegnCd=12`)로 갱신됐으나, 집중률 데이터셋에는 `12`·`46`(구 전남)·`29`(구 광주) 어느 코드로도 데이터가 없다. 여수·완도·고흥 등 낚시 핵심 지역이 통째로 `null`이다. **우리 코드로 해결 불가 — 데이터 제공 측 반영 대기.**
2. **이름 매칭 적중률 — 전국 약 44%** (연안 낚시 스팟 20곳, 반경 5km, 목록 1페이지 237건 기준). 전남을 빼면 53%다. 값이 붙는 비율이 절반에 못 미친다는 것을 전제로 화면을 설계해야 한다.
   - **평균보다 편차가 문제다(0~100%).** 안흥항 83% · 대천항 86% · 거제 86% · 기장 100% · 서귀포 73%로 잘 붙는 곳이 있는 반면, 해운대 16% · 격포항 36% · 여수 0%다.
   - 패턴: **도심일수록 낮고 어촌일수록 높다.** 도심은 TourAPI에 `관광특구`·`○○거리`·`그린레일웨이` 같은 구역·거리 단위 항목이 많이 잡히는데 집중률 데이터셋은 정식 관광지만 다룬다.
3. 미스 패턴이 뚜렷하다 — **항·포구(격포항·한림항·금능포구), 리조트, 신규 시설, 관광특구/거리 단위**. 낚시앱 특성상 자주 노출될 "항"이 데이터셋에 거의 없다.
4. 그 외 연안 시군구 커버리지는 양호: 제주시 244 · 거제 116 · 태안 97 · 강릉 96 · 통영 94 · 강화 87 · 화성 78 · 울주 54 · 속초 50 · 부안 41 · 해운대 19곳.

#### data.go.kr 응답 방어 ⚠️

- **에러 껍데기가 두 가지다.** 정상은 표준 래핑(`response.header`/`response.body`)인데, 파라미터 오류는 래핑 없이 최상위에 `{"resultCode":"11","resultMsg":"..."}`로 온다. 두 형태를 모두 확인하지 않으면 오류를 "결과 0건"으로 착각한다.
- `items.item`은 단건이면 객체 / 다건이면 배열, 결과가 없으면 `items`가 **빈 문자열**이다(§2와 동일).

### 미결정 항목 📋

- [ ] 데이터셋 15101578 활용신청 승인 상태 확인, 운영 쿼터 규모 결정
- [ ] "이미지 있는 장소 우선/필터" 정책(현재는 거리순 그대로, 이미지 유무 무관)
- [ ] 집중률 일 호출 한도 실제 규모 확인·소진 추이 관측 (실시간·무캐시라 쿼터 상향이 전제)
- [ ] 전남 지역 집중률 데이터 제공 여부 재확인(통합 개편 반영 시점)
- [ ] 이름 미매칭 관광지에 대한 대체 표기(빈칸 유지 vs 시군구 평균 등) 정책
