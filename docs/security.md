...
- **공개(인증 불필요):** 인증 API 전체(`/api/auth/**`), 스팟 **상세**(`GET /api/spots/*`)·어종(`GET /api/fish/**`), 랭킹(`GET /api/rankings/**`), 배너(`GET /api/banner/**` — 홈 노출용 계절별 추천 어종), 관광(`GET /api/tours/**` — 위치기반 주변 관광 장소), Swagger(`/swagger-ui/**`·`/v3/api-docs/**`).
