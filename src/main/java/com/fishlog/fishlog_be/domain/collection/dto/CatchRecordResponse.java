 ...
 *
 * <p><b>{@code catchCount}는 자르지 않은 전체 횟수</b>이고 {@code recentCatches}만 최대 4장으로 제한된다. 둘을 같은 값으로 맞추면
 * "더 있다"는 사실이 사라지므로, 프론트는 {@code catchCount - recentCatches.size()}로 "+N장 더" 배지를 그릴 수 있다.
 */
...
public record CatchRecordResponse(
  ...
  public static CatchRecordResponse of(
      String habitat, int catchCount, List<CatchRecord> recentRecords) {
    List<CatchPhotoResponse> photos = recentRecords.stream().map(CatchPhotoResponse::from).toList();
    return new CatchRecordResponse(habitat, catchCount, photos);
  }
}
